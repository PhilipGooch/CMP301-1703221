...
CircleMesh::CircleMesh(ID3D11Device* device, ID3D11DeviceContext* deviceContext, int resolution, float radius) :
...
void CircleMesh::initBuffers(ID3D11Device* device)
{
	VertexType* vertices;
	...
	{
		vertices[i].position = XMFLOAT3(cosf(angle) * radius * 0.6f, sinf(angle) * radius, 0.0f);
	...
	vertices[resolution].normal = XMFLOAT3(0.0f, 0.0f, -1.0f);
